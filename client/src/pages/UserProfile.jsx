  ...
  return (
    <div className="container" style={{ minHeight: "100vh", paddingTop: 26 }}>
      <div style={{ width: "min(980px, 92vw)", margin: "0 auto", display: "grid", gap: 14 }}>
          ...
          <div className="card-inner">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 12 }}>
              <div style={{ border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: 12 }}>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.68)" }}>Wallet balance</div>
                <div style={{ marginTop: 6, fontSize: 20, fontWeight: 900 }}>
                  {stats.walletBalance}{" "}
                  <span style={{ fontSize: 13, fontWeight: 700, color: "rgba(255,255,255,0.68)" }}>credits</span>
                </div>
                <button className="btn" type="button" onClick={() => navigate("/wallet")} style={{ marginTop: 10, width: "100%" }}>
                  Open wallet
                </button>
              </div>

              <div style={{ border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: 12 }}>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.68)" }}>Games created</div>
                <div style={{ marginTop: 6, fontSize: 20, fontWeight: 900 }}>{stats.gamesCreated}</div>
                <div style={{ marginTop: 10, color: "rgba(255,255,255,0.68)", fontSize: 13 }}>Total created via ArenaX</div>
              </div>

              <div style={{ border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: 12 }}>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.68)" }}>Active tournaments</div>
                <div style={{ marginTop: 6, fontSize: 20, fontWeight: 900 }}>{stats.activeTournaments}</div>
                <div style={{ marginTop: 10, color: "rgba(255,255,255,0.68)", fontSize: 13 }}>Live right now</div>
              </div>

              <div style={{ border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: 12 }}>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.68)" }}>Last activity</div>
                <div style={{ marginTop: 6, fontSize: 16, fontWeight: 900 }}>{formatDate(stats.lastActivityAt)}</div>
                <div style={{ marginTop: 10, color: "rgba(255,255,255,0.68)", fontSize: 13 }}>Updated locally</div>
              </div>
            </div>
        ...
        {/* No tournament creation here anymore */}
      </div>
    </div>
  );
}
